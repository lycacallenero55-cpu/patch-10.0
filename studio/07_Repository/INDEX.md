...
> **RECORD — maintained by ④ Repository & Canon Management.** Complete navigation of every tracked file in this repository as of HEAD commit `5284fdd1e58982da9168c76b50134e4878c7d7f2`, grouped by area, with one-line purpose, owning department (per `studio/00_OPERATING_HANDBOOK.md` Amendment 1 §A1.2 write-lane map), and SOURCE_OF_TRUTH classification where that file is registered in `studio/06_Operations/SOURCE_OF_TRUTH.md`. This is a navigation record, not a canon source — where it disagrees with `SOURCE_OF_TRUTH.md` or any file it describes, those files win.
>
> Files added earlier in **CYCLE-0001** are marked **"(new, CYCLE-0001)"**. Files added by **this ④ turn** are marked **"(new, CYCLE-0001 ④)"**.
...
| `studio/00_OPERATING_HANDBOOK.md` | Studio constitution v2.0: **AMENDMENT 1** (binding — five-department model, write-lane ownership map, production cycle, turn mechanics, v1→v2 interpretation map) followed by the retained v1 body (permanent studio laws §3, authority/approval §4, source-of-truth model §5, task system §10, safe git workflow §12, domain workflows §13–19, continuity ledger §20, QA §21, handoff/run-record templates §22, live-mode protocol §11/§24). | ① | `PRODUCTION_SPEC` |
...
| `studio/06_Operations/TASK_QUEUE.md` | Every tracked task with ID/title/status/priority/owner/dependencies/approval-needed/notes. TASK-0015 (this ④ turn) and TASK-0016 (⑤'s parallel cycle-1 audit) both live here. | Shared — any department may append rows during its own turn (A1.2) | `OPERATIONS RECORDS` |
| `studio/06_Operations/DECISION_LOG.md` | Every creator decision (DEC-0001 through DEC-0012 as of this index), with date/decision/decided-by/evidence. DEC-0008/0009 remain PENDING. | ① | `OPERATIONS RECORDS` |
| `studio/06_Operations/OPEN_QUESTIONS.md` | The UNDECIDED ledger, Q-01 through Q-14 as of this index (includes Q-12, the sealed-twist-custody question, and Q-13/Q-14 completeness items). | Shared — any department may append during its own turn (A1.2) | `OPERATIONS RECORDS` |
...
| `studio/06_Operations/HANDOFFS/CYCLE-0001-REPO.md` **(new, CYCLE-0001 ④)** | This ④ turn's handoff (TASK-0015): what was built, CONFLICT-FOR-QA items found, mechanical-fix candidates noticed but not fixed, next department ⑤. | ④ | `OPERATIONS RECORDS` |
| `studio/06_Operations/REVIEWS/` | ⑤'s QA findings directory. Empty as of this index (contains only `.gitkeep`) — ⑤'s CYCLE-0001 audit (TASK-0016) is expected to populate it next. | ⑤ | `OPERATIONS RECORDS` (directory) |
...
## `studio/07_Repository/` — canon registry, index, templates, prompt library, archive (owner: ④) **(new, CYCLE-0001 ④ — this turn)**

| Path | Purpose | Owner | SOURCE_OF_TRUTH class |
|---|---|---|---|
| `studio/07_Repository/CANON_REGISTRY.md` | Item-level canon record register: Characters, Key Objects, Locations/Environments, Powers/Effects, Timeline anchors, Story-visible laws — each entry with a source pointer, status label, and owning department. Records disagreements as CONFLICT-FOR-QA rather than resolving them. Recommend-only; never overrides `SOURCE_OF_TRUTH.md`. | ④ | — (RECORD, indexes canon, is not itself a canon source) |
